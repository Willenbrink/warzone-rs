 ...
 */
    ...
    #[no_mangle]
    fn SendGroupOrderGroup(psGroup: *mut DROID_GROUP, order: DROID_ORDER,
                           x: UDWORD, y: UDWORD, psObj: *mut BASE_OBJECT)
     -> BOOL;
}
/*
 * types.h
...
#[no_mangle]
pub static mut psGrpHeap: *mut OBJ_HEAP =
    0 as *const OBJ_HEAP as *mut OBJ_HEAP;
// initialise the group system
#[no_mangle]
pub unsafe extern "C" fn grpInitialise() -> BOOL {
    if heapCreate(&mut psGrpHeap,
                  ::std::mem::size_of::<DROID_GROUP>() as libc::c_ulong,
                  45 as libc::c_int as UDWORD, 15 as libc::c_int as UDWORD) ==
           0 {
...
#[no_mangle]
pub unsafe extern "C" fn grpShutDown() { heapDestroy(psGrpHeap); }
// create a new group
#[no_mangle]
pub unsafe extern "C" fn grpCreate(mut ppsGroup: *mut *mut DROID_GROUP)
 -> BOOL {
    if heapAlloc(psGrpHeap,
                 ppsGroup as *mut libc::c_void as *mut *mut libc::c_void) == 0
       {
        return 0 as libc::c_int
    }
    (**ppsGroup).type_0 = GT_NORMAL as libc::c_int as SWORD;
    (**ppsGroup).refCount = 0 as libc::c_int as SWORD;
    (**ppsGroup).psList = 0 as *mut DROID;
    (**ppsGroup).psCommander = 0 as *mut DROID;
    memset(&mut (**ppsGroup).sRunData as *mut RUN_DATA as *mut libc::c_void,
           0 as libc::c_int,
...
#[no_mangle]
pub unsafe extern "C" fn grpJoin(mut psGroup: *mut DROID_GROUP,
                                 mut psDroid: *mut DROID) {
    (*psGroup).refCount =
        ((*psGroup).refCount as libc::c_int + 1 as libc::c_int) as SWORD;
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              b"grpJoin: invalid group pointer\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              b"Assert in Warzone: %s:%d : %s (%s)\x00" as *const u8 as
                  *const libc::c_char,
              b"group.c\x00" as *const u8 as *const libc::c_char,
              66 as libc::c_int,
              (*::std::mem::transmute::<&[u8; 8],
                                        &[libc::c_char; 8]>(b"grpJoin\x00")).as_ptr(),
    ...
    if !psDroid.is_null() {
        if !(*psGroup).psList.is_null() &&
               (*psDroid).player as libc::c_int !=
                   (*(*psGroup).psList).player as libc::c_int {
            if 0 as libc::c_int != 0 {
            } else {
                debug(LOG_ERROR,
                      b"grpJoin: Cannot have more than one players droids in a group\x00"
        ...
        if (*psDroid).droidType as libc::c_uint ==
            ...
            if (*psGroup).type_0 as libc::c_int == GT_NORMAL as libc::c_int {
            } else {
                debug(LOG_ERROR,
                      b"grpJoin: Cannot have two transporters in a group\x00"
                          as *const u8 as *const libc::c_char);
            };
            if (*psGroup).type_0 as libc::c_int == GT_NORMAL as libc::c_int {
            } else {
                debug(LOG_ERROR,
                      ...
                      (*::std::mem::transmute::<&[u8; 8],
                                                &[libc::c_char; 8]>(b"grpJoin\x00")).as_ptr(),
                      b"(psGroup->type == GT_NORMAL)\x00" as *const u8 as
                          *const libc::c_char);
            };
            (*psGroup).type_0 = GT_TRANSPORTER as libc::c_int as SWORD;
            ...
            (*psGroup).psList = psDroid
                      ...
                      (*psGroup).type_0 as libc::c_int !=
                          GT_TRANSPORTER as libc::c_int {
            if (*psGroup).type_0 as libc::c_int == GT_NORMAL as libc::c_int &&
                   (*psGroup).psCommander.is_null() {
            } else {
                debug(LOG_ERROR,
                      b"grpJoin: Cannot have two command droids in a group\x00"
                          as *const u8 as *const libc::c_char);
            };
            if (*psGroup).type_0 as libc::c_int == GT_NORMAL as libc::c_int &&
                   (*psGroup).psCommander.is_null() {
                ...
                debug(LOG_ERROR,
                      ...
                      (*::std::mem::transmute::<&[u8; 8],
                                                &[libc::c_char; 8]>(b"grpJoin\x00")).as_ptr(),
                      b"(psGroup->type == GT_NORMAL) && (psGroup->psCommander == NULL)\x00"
                          as *const u8 as *const libc::c_char);
            };
            (*psGroup).type_0 = GT_COMMAND as libc::c_int as SWORD;
...
#[no_mangle]
pub unsafe extern "C" fn grpJoinEnd(mut psGroup: *mut DROID_GROUP,
                                    mut psDroid: *mut DROID) {
    ...
    if 1 as libc::c_int != 0 {
        ...
        debug(LOG_ERROR,
              ...
              (*::std::mem::transmute::<&[u8; 11],
                                        &[libc::c_char; 11]>(b"grpJoinEnd\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    // if psDroid == NULL just increase the refcount don't add anything to the list
    if !psDroid.is_null() {
        ...
        if (*psDroid).droidType as libc::c_uint ==
            ...
            } else { (*psGroup).psList = psDroid }
        }
    };
}
// remove a droid from a group
#[no_mangle]
pub unsafe extern "C" fn grpLeave(mut psGroup: *mut DROID_GROUP,
                                  mut psDroid: *mut DROID) {
    let mut psPrev: *mut DROID = 0 as *mut DROID;
    let mut psCurr: *mut DROID = 0 as *mut DROID;
    if 1 as libc::c_int != 0 {
        ...
        debug(LOG_ERROR,
              b"grpLeave: invalid group pointer\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              ...
              (*::std::mem::transmute::<&[u8; 9],
                                        &[libc::c_char; 9]>(b"grpLeave\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    if !psDroid.is_null() && (*psDroid).psGroup != psGroup {
        if 0 as libc::c_int != 0 {
        } else {
            debug(LOG_ERROR,
                  b"grpLeave: droid group does not match\x00" as *const u8 as
                      *const libc::c_char);
        };
        if 0 as libc::c_int != 0 {
        } else {
    ...
    if !psDroid.is_null() &&
           ((*psDroid).droidType as libc::c_uint !=
                DROID_COMMAND as libc::c_int as libc::c_uint ||
                (*psGroup).type_0 as libc::c_int != GT_COMMAND as libc::c_int)
       {
        psPrev = 0 as *mut DROID;
        psCurr = (*psGroup).psList;
        while !psCurr.is_null() {
            if psCurr == psDroid { break ; }
            psPrev = psCurr;
            psCurr = (*psCurr).psGrpNext
        }
        if !psCurr.is_null() {
            ...
            debug(LOG_ERROR,
                  b"grpLeave: droid not found\x00" as *const u8 as
                      *const libc::c_char);
        };
        if !psCurr.is_null() {
        } else {
        ...
        if !psCurr.is_null() {
            ...
            } else { (*psGroup).psList = (*(*psGroup).psList).psGrpNext }
        }
    }
    if !psDroid.is_null() {
        ...
        if (*psDroid).droidType as libc::c_uint ==
               DROID_COMMAND as libc::c_int as libc::c_uint &&
               (*psGroup).type_0 as libc::c_int == GT_COMMAND as libc::c_int {
            (*psGroup).type_0 = GT_NORMAL as libc::c_int as SWORD;
            (*psGroup).psCommander = 0 as *mut DROID
        } else if (*psDroid).droidType as libc::c_uint ==
                      DROID_TRANSPORTER as libc::c_int as libc::c_uint &&
                      (*psGroup).type_0 as libc::c_int ==
                          GT_TRANSPORTER as libc::c_int {
    ...
    if (*psGroup).refCount as libc::c_int <= 0 as libc::c_int {
        heapFree(psGrpHeap, psGroup as *mut libc::c_void);
    };
}
/*
 * Group.h
...
// count the members of a group
#[no_mangle]
pub unsafe extern "C" fn grpNumMembers(mut psGroup: *mut DROID_GROUP)
 -> SDWORD {
    let mut psCurr: *mut DROID = 0 as *mut DROID;
    ...
    if 1 as libc::c_int != 0 {
        ...
        debug(LOG_ERROR,
              b"grpNumMembers: invalid droid group\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              ...
              (*::std::mem::transmute::<&[u8; 14],
                                        &[libc::c_char; 14]>(b"grpNumMembers\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    num = 0 as libc::c_int;
...
// remove all droids from a group
#[no_mangle]
pub unsafe extern "C" fn grpReset(mut psGroup: *mut DROID_GROUP) {
    let mut psCurr: *mut DROID = 0 as *mut DROID;
    let mut psNext: *mut DROID = 0 as *mut DROID;
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              b"grpReset: invalid droid group\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              ...
              260 as libc::c_int,
              (*::std::mem::transmute::<&[u8; 9],
                                        &[libc::c_char; 9]>(b"grpReset\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    psCurr = (*psGroup).psList;
...
/* Give a group an order */
#[no_mangle]
pub unsafe extern "C" fn orderGroup(mut psGroup: *mut DROID_GROUP,
                                    mut order: DROID_ORDER) {
    let mut psCurr: *mut DROID = 0 as *mut DROID;
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              b"orderGroup: invalid droid group\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              ...
              305 as libc::c_int,
              (*::std::mem::transmute::<&[u8; 11],
                                        &[libc::c_char; 11]>(b"orderGroup\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    psCurr = (*psGroup).psList;
...
/* Give a group of droids an order */
#[no_mangle]
pub unsafe extern "C" fn orderGroupLoc(mut psGroup: *mut DROID_GROUP,
    ...
    if 1 as libc::c_int != 0 {
        ...
        debug(LOG_ERROR,
              b"orderGroupLoc: invalid droid group\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              ...
              319 as libc::c_int,
              (*::std::mem::transmute::<&[u8; 14],
                                        &[libc::c_char; 14]>(b"orderGroupLoc\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    if bMultiPlayer != 0 {
        SendGroupOrderGroup(psGroup, order, x, y, 0 as *mut BASE_OBJECT);
...
/* Give a group of droids an order */
#[no_mangle]
pub unsafe extern "C" fn orderGroupObj(mut psGroup: *mut DROID_GROUP,
                                       mut order: DROID_ORDER,
    ...
    if 1 as libc::c_int != 0 {
        ...
        debug(LOG_ERROR,
              b"orderGroupObj: invalid droid group\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              ...
              349 as libc::c_int,
              (*::std::mem::transmute::<&[u8; 14],
                                        &[libc::c_char; 14]>(b"orderGroupObj\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    if bMultiPlayer != 0 {
        SendGroupOrderGroup(psGroup, order, 0 as libc::c_int as UDWORD,
...
/* set the secondary state for a group of droids */
#[no_mangle]
pub unsafe extern "C" fn grpSetSecondary(mut psGroup: *mut DROID_GROUP,
    ...
    if 1 as libc::c_int != 0 {
        ...
        debug(LOG_ERROR,
              b"grpSetSecondary: invalid droid group\x00" as *const u8 as
                  *const libc::c_char);
    };
    if 1 as libc::c_int != 0 {
    } else {
        debug(LOG_ERROR,
              ...
              (*::std::mem::transmute::<&[u8; 16],
                                        &[libc::c_char; 16]>(b"grpSetSecondary\x00")).as_ptr(),
              b"PTRVALID(psGroup, sizeof(DROID_GROUP))\x00" as *const u8 as
                  *const libc::c_char);
    };
    psCurr = (*psGroup).psList;
