 ...
 */
    ...
    #[no_mangle]
    fn memcmp(_: *const libc::c_void, _: *const libc::c_void, _: libc::c_uint)
     -> libc::c_int;
    #[no_mangle]
 ...
 */
    ...
    #[no_mangle]
    static mut cheatHash: [UDWORD; 29];
...
// variables
    ...
    #[no_mangle]
    fn chooseColour(_: UDWORD) -> BOOL;
    #[no_mangle]
    fn kickPlayer(dpid: DPID);
    #[no_mangle]
    fn addPlayerBox(_: BOOL) -> UDWORD;
    // form disk
    #[no_mangle]
 ...
 */
    // for loading maps
    // for droid_order_data
    // ////////////////////////////////////////////////////////////////////////////
// External Variables
    #[no_mangle]
    static mut bHosted: BOOL;
    #[no_mangle]
...
// Version Check
#[no_mangle]
pub unsafe extern "C" fn sendVersionCheck() -> BOOL {
    let mut msg: NETMSG =
        NETMSG{size: 0,
               paddedBytes: 0,
               type_0: 0,
    ...
    memcpy(&mut *msg.body.as_mut_ptr().offset(1 as libc::c_int as isize) as
               *mut libc::c_char as *mut libc::c_void,
           &mut cheatHash as *mut [UDWORD; 29] as *const libc::c_void,
           ::std::mem::size_of::<[UDWORD; 29]>() as libc::c_ulong);
    msg.size =
        (::std::mem::size_of::<[UDWORD; 29]>() as
             libc::c_ulong).wrapping_add(1 as libc::c_int as libc::c_uint) as
            libc::c_ushort;
    msg.type_0 = NET_VERSION as libc::c_int as libc::c_uchar;
    return NETbcast(&mut msg, 1 as libc::c_int);
}
#[no_mangle]
pub unsafe extern "C" fn recvVersionCheck(mut pMsg: *mut NETMSG) -> BOOL {
    let mut extCheat: [UDWORD; 29] = [0; 29];
    let mut pl: UBYTE = 0;
    let mut sTmp: [CHAR; 128] = [0; 128];
    memcpy(&mut extCheat as *mut [UDWORD; 29] as *mut libc::c_void,
           &mut *(*pMsg).body.as_mut_ptr().offset(1 as libc::c_int as isize)
               as *mut libc::c_char as *const libc::c_void,
           ::std::mem::size_of::<[UDWORD; 29]>() as libc::c_ulong);
    if memcmp(extCheat.as_mut_ptr() as *const libc::c_void,
              cheatHash.as_mut_ptr() as *const libc::c_void,
              (29 as libc::c_int * 4 as libc::c_int) as libc::c_uint) !=
           0 as libc::c_int {
        // what should we do now ?
        memcpy(&mut pl as *mut UBYTE as *mut libc::c_void,
        ...
        sprintf(sTmp.as_mut_ptr(),
                b"%s has different data. CHEATING or wrong version\x00" as
                    *const u8 as *const libc::c_char,
                getPlayerName(pl as UDWORD));
        addConsoleMessage(sTmp.as_mut_ptr(), DEFAULT_JUSTIFY);
        sendTextMessage(sTmp.as_mut_ptr(), 1 as libc::c_int);
        /*
...
// Wait For Players
#[no_mangle]
pub unsafe extern "C" fn intDisplayMultiJoiningStatus(mut joinCount: UBYTE)
 -> BOOL {
    let mut x: UDWORD = 0;
    let mut y: UDWORD = 0;
    let mut w: UDWORD = 0;
    let mut h: UDWORD = 0;
    let mut sTmp: [CHAR; 6] = [0; 6];
    w = 132 as libc::c_int as UDWORD;
    h = 132 as libc::c_int as UDWORD;
    x = 23 as libc::c_int as UDWORD;
    y =
    ...
    RenderWindowFrame(&mut FrameNormal, x, y, w, h); // draw a wee blu box.
    // display how far done..
    pie_DrawText(strresGetString(psStringRes,
                                 STR_GAM_JOINING as libc::c_int as UDWORD),
                 x.wrapping_add(w.wrapping_div(2 as libc::c_int as
                                                   libc::c_uint)).wrapping_sub((iV_GetTextWidth(strresGetString(psStringRes,
                                                                                                                STR_GAM_JOINING
                                                                                                                    as
                                                                                                                    libc::c_int
                                                                                                                    as
                                                                                   ...
                                                                                   as
                                                                                   libc::c_uint),
                 y.wrapping_add(h.wrapping_div(2 as libc::c_int as
    ...
    sprintf(sTmp.as_mut_ptr(),
            ...
            NetPlay.playercount.wrapping_sub(joinCount as
                                                 libc::c_uint).wrapping_mul(100
                                                                                as
                                                                                libc::c_int
                                                                                as
                                                                                libc::c_uint).wrapping_div(NetPlay.playercount));
    pie_DrawText(sTmp.as_mut_ptr(),
                 x.wrapping_add(w.wrapping_div(2 as libc::c_int as
...
#[no_mangle]
pub unsafe extern "C" fn clearPlayer(mut player: UDWORD, mut quietly: BOOL,
                                     mut removeOil: BOOL) {
    let mut i: UDWORD = 0; // remove player, make computer controlled
    let mut bTemp: BOOL = 0; // if they never joined, reset the flag
    let mut psStruct: *mut STRUCTURE = 0 as *mut STRUCTURE;
    let mut psNext: *mut STRUCTURE = 0 as *mut STRUCTURE;
    player2dpid[player as usize] = 0 as libc::c_int;
    ingame.JoiningInProgress[player as usize] = 0 as libc::c_int;
    i = 0 as libc::c_int as UDWORD;
    while i < 8 as libc::c_int as libc::c_uint {
        // remove alliances
        alliances[player as usize][i as usize] = 0 as libc::c_int as UBYTE;
        alliances[i as usize][player as usize] = 0 as libc::c_int as UBYTE;
        i = i.wrapping_add(1)
    }
    while !apsDroidLists[player as usize].is_null() {
        // delete all droids
        if quietly != 0 {
            killDroid(apsDroidLists[player as usize]);
        } else { destroyDroid(apsDroidLists[player as usize]); }
    }
    ...
    while !psStruct.is_null() {
        ...
        if bTemp != 0 {
            if (*(*apsFeatureLists[0 as libc::c_int as
                                       usize]).psStats).subType as
                   libc::c_uint ==
                   FEAT_OIL_RESOURCE as libc::c_int as libc::c_uint {
                removeFeature(apsFeatureLists[0 as libc::c_int as usize]);
            }
        }
        psStruct = psNext
    };
...
#[no_mangle]
pub unsafe extern "C" fn resetMultiVisibility(mut player: UDWORD) {
    let mut owned: UDWORD = 0;
    let mut pDroid: *mut DROID = 0 as *mut DROID;
    let mut pStruct: *mut STRUCTURE = 0 as *mut STRUCTURE;
    owned = 0 as libc::c_int as UDWORD;
    while owned < 8 as libc::c_int as libc::c_uint {
        // for each player
        if owned != player {
            // done reset own stuff..
            //droids
            pDroid = apsDroidLists[owned as usize];
            while !pDroid.is_null() {
                (*pDroid).visible[player as usize] =
                    0 as libc::c_int as UBYTE;
                pDroid = (*pDroid).psNext
            }
            pStruct = apsStructLists[owned as usize];
            while !pStruct.is_null() {
                (*pStruct).visible[player as usize] =
                    0 as libc::c_int as UBYTE;
                pStruct = (*pStruct).psNext
            }
        }
        owned = owned.wrapping_add(1)
    };
}
//structures
...
#[no_mangle]
pub unsafe extern "C" fn MultiPlayerLeave(mut dp: DPID) -> BOOL {
    let mut i: UDWORD = 0 as libc::c_int as UDWORD; // find out which!
    let mut buf: [libc::c_char; 255] = [0; 255];
    while player2dpid[i as usize] != dp &&
              i < 8 as libc::c_int as libc::c_uint {
        i = i.wrapping_add(1)
    }
    if i != 8 as libc::c_int as libc::c_uint {
        // player not already removed
        NETlogEntry(b"Player Unexpectedly leaving, came from directplay...\x00"
                        as *const u8 as *const libc::c_char as *mut CHAR,
                    0 as libc::c_int as UDWORD,
                    dp as UDWORD); // update the player info stuff
        sprintf(buf.as_mut_ptr(),
                strresGetString(psStringRes,
                                STR_MUL_LEAVE as libc::c_int as UDWORD),
                getPlayerName(i));
        turnOffMultiMsg(1 as libc::c_int);
        clearPlayer(i, 0 as libc::c_int, 0 as libc::c_int);
        game.skDiff[i as usize] = 10 as libc::c_int as UBYTE;
        turnOffMultiMsg(0 as libc::c_int);
        addConsoleMessage(buf.as_mut_ptr(), DEFAULT_JUSTIFY);
        if !widgGetFromID(psWScreen, 1 as libc::c_int as UDWORD).is_null() {
            audio_QueueTrack(ID_CLAN_EXIT as libc::c_int);
        }
    }
    NETplayerInfo();
    // fire script callback to reassign skirmish players.
    eventFireCallbackTrigger(CALL_PLAYERLEFT as libc::c_int as TRIGGER_TYPE);
    return 1 as libc::c_int;
}
// A player has left the game.
// remote player has left.
// ////////////////////////////////////////////////////////////////////////////
// A Remote Player has joined the game.
#[no_mangle]
pub unsafe extern "C" fn MultiPlayerJoin(mut dpid: DPID) -> BOOL {
    let mut i: UDWORD = 0;
    if !widgGetFromID(psWScreen, 1 as libc::c_int as UDWORD).is_null() {
        // if ingame.
        audio_QueueTrack(ID_CLAN_ENTER as libc::c_int);
    }
    if !widgGetFromID(psWScreen, 10231 as libc::c_int as UDWORD).is_null() {
        // if in multimenu.
        if multiRequestUp == 0 &&
               (bHosted != 0 ||
    ...
    if NetPlay.bHost != 0 {
        // host responsible for welcoming this player.
        // if we've already received a request from this player don't reallocate.
        i = 0 as libc::c_int as UDWORD;
        while i < 8 as libc::c_int as libc::c_uint {
            if player2dpid[i as usize] == dpid &&
                   ingame.JoiningInProgress[i as usize] != 0 {
                return 1 as libc::c_int
            }
            i = i.wrapping_add(1)
        }
        if NetPlay.playercount <= 8 as libc::c_int as libc::c_uint {
        } else {
            debug(LOG_ERROR,
                  b"Too many players!\x00" as *const u8 as
                      *const libc::c_char);
        };
        if NetPlay.playercount <= 8 as libc::c_int as libc::c_uint {
        } else {
            debug(LOG_ERROR,
                  b"Assert in Warzone: %s:%d : %s (%s)\x00" as *const u8 as
                      *const libc::c_char,
                  b"multijoin.c\x00" as *const u8 as *const libc::c_char,
                  317 as libc::c_int,
                  (*::std::mem::transmute::<&[u8; 16],
                                            &[libc::c_char; 16]>(b"MultiPlayerJoin\x00")).as_ptr(),
                  b"NetPlay.playercount<=MAX_PLAYERS\x00" as *const u8 as
                      *const libc::c_char);
        };
        loop 
             ...
             {
            i =
                (rand() % game.maxPlayers as libc::c_int) as
                    UDWORD; // force a colourchoice
            if !(player2dpid[i as usize] != 0 as libc::c_int) {
                break ; // pick an unused colour.
            }
        } // setup all the guff for that player.
        setPlayerColour(i, 8 as libc::c_int as UDWORD);
        chooseColour(i);
        setupNewPlayer(dpid, i);
        if NetPlay.bLobbyLaunched == 0 ||
               NetPlay.bLobbyLaunched != 0 && bHosted != 0 {
            sendOptions(dpid, i);
        }
        if game.type_0 as libc::c_int == 14 as libc::c_int &&
               NetPlay.playercount > game.maxPlayers as libc::c_uint {
            kickPlayer(dpid);
        }
    }
    return 1 as libc::c_int;
...
// remote player has just joined.
// ////////////////////////////////////////////////////////////////////////////
// Setup Stuff for a new player.
#[no_mangle]
pub unsafe extern "C" fn setupNewPlayer(mut dpid: DPID, mut player: UDWORD) {
    let mut i: UDWORD = 0; //,col;
    let mut buf: [libc::c_char; 255] = [0; 255]; // assign them a player too.
    player2dpid[player as usize] = dpid; // reset ping time
    ingame.PingTimes[player as usize] =
        0 as libc::c_int as UDWORD; // note that player is now joining.
    ingame.JoiningInProgress[player as usize] = 1 as libc::c_int;
    //	if (game.type == DMATCH)
//	{
...
//	}
    i = 0 as libc::c_int as UDWORD;
    while i < 8 as libc::c_int as libc::c_uint {
        // set all alliances to broken.
        alliances[selectedPlayer as usize][i as usize] =
            0 as libc::c_int as UBYTE; // set visibility flags.
        alliances[i as usize][selectedPlayer as usize] =
            0 as libc::c_int as UBYTE; // update the net info stuff
        i = i.wrapping_add(1)
    } // get the players score from the ether.
    resetMultiVisibility(player);
    NETplayerInfo();
    setMultiStats(player2dpid[player as usize],
                  getMultiStats(player, 0 as libc::c_int), 1 as libc::c_int);
    sprintf(buf.as_mut_ptr(),
            strresGetString(psStringRes,
                            STR_MUL_JOINING as libc::c_int as UDWORD),
...
#[no_mangle]
pub unsafe extern "C" fn modifyResources(mut psFunction:
                                             *mut POWER_GEN_FUNCTION) {
    match game.power {
        100 => {
            (*psFunction).powerMultiplier =
                (*psFunction).powerMultiplier.wrapping_mul(3 as libc::c_int as
        ...
        400 => {
            (*psFunction).powerMultiplier =
                (*psFunction).powerMultiplier.wrapping_mul(1 as libc::c_int as
                                                               libc::c_uint)
        }
        700 => {
            (*psFunction).powerMultiplier =
                (*psFunction).powerMultiplier.wrapping_mul(5 as libc::c_int as
                                                               libc::c_uint).wrapping_div(4
                                                                                              as
                                                                                              libc::c_int
                                                                                              as
        ...
        _ => { }
    }; // careful with the brackets! (do mul before div)
}
